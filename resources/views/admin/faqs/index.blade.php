...
@section('content')
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
            ...
            <h1 class="mt-2 text-3xl font-semibold text-slate-900">FAQ</h1>
            <p class="mt-2 text-sm text-slate-600">Pertanyaan umum yang tampil di homepage.</p>
        </div>
        <div class="flex flex-wrap gap-3">
            <form method="GET" action="{{ route('admin.faqs.index') }}" class="flex flex-wrap gap-3">
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari pertanyaan..." class="w-64 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm" />
                <select name="language" class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm">
            ...
            <a href="{{ route('admin.faqs.create') }}" class="rounded-full bg-emerald-600 px-5 py-3 text-sm font-semibold text-white">Tambah</a>
        </div>
    </div>
            ...
            <tbody>
                @forelse ($faqs as $faq)
                    <tr class="border-t border-slate-100">
                        ...
                        <td class="px-5 py-4 text-slate-700">{{ strtoupper($faq->language_code) }}</td>
                        <td class="px-5 py-4 text-slate-700">{{ $faq->sort_order }}</td>
                        <td class="px-5 py-4">
                            @if ($faq->is_active)
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('admin.faqs.edit', $faq) }}" class="text-emerald-700 hover:text-emerald-800">Edit</a>
